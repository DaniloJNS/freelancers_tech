...
describe Project do
  context 'belong_to' do
    ...
    it 'user must exists' do
      project.valid?
      expect(project.errors.full_messages_for(:user)).to include(
  ...
  context 'validates' do
    ...
    context 'cant be blank' do
      ...
      it 'max_price_per_hour' do
        project.valid?
        expect(project.errors.full_messages_for(:max_price_per_hour)).to include(
          'Preço máximo por hora não pode ficar em branco') 
      end
      it 'deadline_submission' do
        project.valid?
        expect(project.errors.full_messages_for(:deadline_submission)).to include(
          'Prazo para submissão não pode ficar em branco') 
      end
    end
    context 'must be greater_than 0' do
      it 'max_price_per_hour' do
        ...
        project.valid?
        expect(project.errors.full_messages_for(:max_price_per_hour)).to include(
          'Preço máximo por hora deve ser maior que 0') 
      end
    end
